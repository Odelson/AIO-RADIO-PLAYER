<?php
    ...
    if ( is_dir( './../inc/lang' ) ) {
        ...
        // Only available
        foreach ( $ff as $file ) {
            $languages[ $file ] = $language[ extDel( $file ) ] . ' (' . strtoupper( extDel( $file ) ) . ')';
        }
    ...
    $fields_arr = array(
        ...
        array( 'label' => 'Artist images API', 'class' => 'col-sm-4', 'name' => 'api_artist', 'type' => 'select', 'options' => array( 'lastfm' => 'LastFM', 'echonest' => 'EchoNest', 'itunes' => 'iTunes ( no API key )', 'custom' => 'Custom ( URL )', 'disabled' => 'Disabled' ) ),
        ...
        array( 'label' => 'Artist image URL', 'name' => 'artist_api_url', 'class' => 'col-sm-6', 'description' => '( Available variables: {$ARTIST} and {$TITLE} )', 'placeholder' => 'http://website.com/?artist={$ARTIST}&title={$TITLE}' ),
        ...
        array( 'label' => 'Panel username', 'name' => 'admin_user', 'class' => 'col-sm-4', 'placeholder' => 'admin', 'autocomplete' => 'new-username' ),
        array( 'label' => 'Panel password', 'name' => 'admin_pass', 'class' => 'col-sm-4', 'type' => 'password', 'placeholder' => 'min. 5 characters', 'autocomplete' => 'new-password' ),
        array( 'label' => 'Confirm panel password', 'name' => 'admin_pass2', 'class' => 'col-sm-4', 'type' => 'password', 'placeholder' => 'min. 5 characters', 'autocomplete' => 'new-alt-password' )
...
<script type="text/javascript">
    ...
    function optionalFields() {
        ...
        var v = $( '#api_artist' ).val();

        // Check if lastfm/echonest
        if ( v == 'lastfm' || v == 'echonest' ) {

            $( '#api_key' ).closest( '.form-group' ).show();
            $( '#artist_api_url' ).closest( '.form-group' ).hide();

        } else if ( v == 'custom' ) {

            $( '#artist_api_url' ).closest( '.form-group' ).show();
    ...
    $( document ).ready( function() {
        ...
        $( '#api_artist' ).on( 'change', optionalFields );
        optionalFields();
