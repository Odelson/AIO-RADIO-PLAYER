<?php
    ...
    if ( isset( $_POST[ 'submit' ] ) AND $_POST[ 'submit' ] == 'compile' ) {
    ...
    if ( isset( $_POST[ 'submit' ] ) AND $_POST[ 'submit' ] == 'upload' ) {
        ...
        if ( !in_array( extGet( $_FILES[ 'image' ][ 'name' ] ), $allow_ext ) ) {
            ...
            $artist = parseTrack( $_POST[ 'artist' ] );
            deleteArtist( $_POST[ 'artist' ] );
...
<div class="panel">
    ...
    <div class="content">
        ...
        <pre class="debug-output commands-pre" style="display: none; margin-top: 15px; margin-bottom: 0;"></pre>
        <iframe id="debug-iframe" src="about:blank" style="border:0;" border="0" width="0" height="0"></iframe>
    </div>
</div>

<?php echo ( !empty( $theme_message ) ) ? $theme_message : ''; ?>
...
<?php echo ( !empty( $artwork_upload ) ) ? $artwork_upload : ''; ?>
