<?php
     ...
     */
    function readStream( $stream_uri ) {
    ...
    function upload( $form_name, $path = 'data/uploads/', $filename = '' ) {
        ...
        // Extension variable
        $extension = extGet( $_FILES[ $form_name ][ 'name' ] );
     ...
     * @return mixed
     */
    function api_lastfm( $artist, $title = '', $s = array() ) {

        $data = xml2array( get( "http://ws.audioscrobbler.com/2.0/?method=artist.getinfo&artist=" . urlencode( $artist ) . "&api_key={$s['api_key']}", false, false, false, 30 ) );
     ...
     * @return mixed
     */
    function api_echonest( $artist, $title = '', $s = array() ) {

        $data = json_decode( get( "http://developer.echonest.com/api/v4/artist/images?api_key={$s['api_key']}&name=" . urlencode( $artist ) . "&format=json&results=1&start=0", false, false, false, 30 ), true );
        return $data[ 'response' ][ 'images' ][ 0 ][ 'url' ];

    }

    /**
     * Doc: itunes.apple.com
     *
     * @param  $artist
     * @param  $title
     * @param  $s
     ...
     * @return bool|string
     */
    function api_itunes( $artist, $title = '', $s = array() ) {

        // Attempt searching for image
        $data = get( 'http://itunes.apple.com/search?term=' . urlencode( $artist ) . '&attribute=allArtistTerm&entity=musicTrack&limit=1' );
        ...
        if ( $data !== false ) {
            ...
            if ( $data[ 'resultCount' ] >= 1 ) { // Check if result is not empty
                ...
                return substr( $data[ 'results' ][ 0 ][ 'artworkUrl100' ], 0, $last_slash ) . "/{$s['images_size']}x{$s['images_size']}." . extGet( $data[ 'results' ][ 0 ][ 'artworkUrl100' ] );
     ...
     * @return mixed
     */
    function api_custom( $artist, $title = '', $s = array() ) {

        return ( filter_var( $s[ 'artist_api_url' ], FILTER_VALIDATE_URL ) ) ?
            template( $s[ 'artist_api_url' ], array( 'artist' => urlencode( $artist ), 'title' => urlencode( $title ) ) ) : false;
    ...
    function getArtwork( $artist, $title = '', $settings = array() ) {
        ...
        // Few variables =)
        $extensions      = array( 'jpg', 'jpeg', 'png', 'svg', 'webp' ); ## Allowed image extensions
        $artist_name     = parseTrack( $artist );                 ## AIO Encodes artist name into nice readable string
        $track_name      = parseTrack( "{$artist} - {$title}" );  ## AIO Encodes artist & title into nice readable string
        $default_artwork = false;

        // Check image size (since 1.31)
        if ( !isset( $settings[ 'images_size' ] ) || !is_numeric( $settings[ 'images_size' ] ) || $settings[ 'images_size' ] < 100 )
        ...
        foreach ( $extensions as $ext ) {
            ...
            if ( is_file( "tmp/images/default.{$ext}" ) ) {
                ...
                if ( empty( $artist ) OR strlen( $artist ) < 3 ) {
                    ...
                    // This is used when artwork is not found in cache/artwork folder/online
        ...
        foreach ( $extensions as $ext ) { ## Images saved in /tmp/images/ have higher priority than cached images
            if ( is_file( "tmp/images/{$track_name}.{$ext}" ) ) return "tmp/images/{$track_name}.{$ext}";
        }

        // Pre-defined artist images
        foreach ( $extensions as $ext ) { ## Images saved in /tmp/images/ have higher priority than cached images
            if ( is_file( "tmp/images/{$artist_name}.{$ext}" ) ) return "tmp/images/{$artist_name}.{$ext}";
        }

        // Special case, when artwork is disabled, do not use return cached
        if ( $settings[ 'api_artist' ] == 'disabled' )
            return $default_artwork;

        // Cached version available
        foreach ( $extensions as $ext ) { ## Low priority cached images from various sources
            if ( is_file( "tmp/cache/{$artist_name}.{$ext}" ) ) return "tmp/cache/{$artist_name}.{$ext}";
        }
        ...
        if ( isset( $settings[ 'radionomy' ] ) && $settings[ 'radionomy' ] == 'true' && filter_var( $settings[ 'artwork' ], FILTER_VALIDATE_URL ) ) {
            ...
            $imageURL = $api_func( trim( $artist ), trim( $title ), $settings );
        ...
        if ( $imageURL === false || !filter_var( $imageURL, FILTER_VALIDATE_URL ) ) {
            ...
            $img = get(
                ...
                array(
                    CURLOPT_HEADERFUNCTION => function( $ch, $hLine ) use ( &$imageType ) {
                        ...
                        if ( stripos( $hLine, 'content-type' ) !== false ) {
                            if ( stripos( $hLine, 'image/' ) !== false ) {
            ...
            if ( strlen( $img ) < 1024 || !in_array( $imageType, $extensions ) ) return $default_artwork;

            // Download image into tmp/cache/ directory (include its extension)
            $path = "tmp/cache/{$artist_name}." . $imageType;
    ...
    function deleteArtist( $name ) {
        ...
        if ( is_array( $files ) ) {
            ...
            foreach ( $files as $file ) { // Loop files
                ...
                if ( extDel( $file ) == $name ) { // File matches, delete all extensions of this artist

                    foreach ( $allow_ext as $ext ) {

                        // Delete file
                        if ( is_file( "./../tmp/images/" . extDel( $file ) . ".{$ext}" ) )
                            @unlink( "./../tmp/images/" . extDel( $file ) . ".{$ext}" );
