    ...
    switch ( $channel[ 'stats' ][ 'method' ] ) {
        ...
        case 'direct':
            ...
            if ( !$info = $cache->get( 'stream.' . $chn_key . '.info' ) ) {
                ...
                preg_match( '/' . $settings[ 'track_regex' ] . '/', strToUTF8( $get_info ), $track );
        ...
        case 'shoutcast':
            ...
            if ( !$info = $cache->get( 'stream.' . $chn_key . '.info' ) ) {
                ...
                if ( !$xmlfile = get( "{$channel['stats']['url']}/admin.cgi?pass={$channel['stats']['auth']}&mode=viewxml&sid={$channel['stats']['sid']}" ) ) {
                    ...
                    // Now, result must not be empty or err occurred
                    preg_match( '/' . $settings[ 'track_regex' ] . '/', strToUTF8( $sc_data[ 'songtitle' ] ), $track );
                    ...
                    if ( $channel[ 'stats' ][ 'sc-history' ] && isset( $sc_data[ 'songhistory' ] ) && count( $sc_data[ 'songhistory' ][ 'SONG' ] ) >= 1 ) {
                        ...
                        foreach ( $sc_data[ 'songhistory' ][ 'SONG' ] as $song ) {
                            ...
                            preg_match( '/' . $settings[ 'track_regex' ] . '/', strToUTF8( $song[ 'TITLE' ] ), $sc );
        ...
        case 'icecast':
            ...
            if ( !$info = $cache->get( 'stream.' . $chn_key . '.info' ) ) {
                ...
                if ( !$xmlfile = get( "{$channel['stats']['url']}/admin/stats", null, "{$channel['stats']['auth-user']}:{$channel['stats']['auth-pass']}" ) ) {
                    ...
                    $ice = array();
                    $iceD = xml2array( $xmlfile, true );

                    // Multiple mount points
                    if ( is_array( $iceD[ 'source' ][ 0 ] ) ) {

                        foreach ( $iceD[ 'source' ] as $mount ) {
                        ...
                        // Get mount name
                        $mountName = $iceD[ 'source' ][ '@attributes' ][ 'mount' ];
                        unset( $iceD[ 'source' ][ '@attributes' ] );

                        // Set mount info
                        $ice[ $mountName ] = $iceD[ 'source' ];
                    ...
                    if ( !is_array( $ice[ $channel[ 'stats' ][ 'mount' ] ] ) AND !is_array( $ice[ $channel[ 'stats' ][ 'fallback' ] ] ) ) {
                        ...
                        preg_match( '/' . $settings[ 'track_regex' ] . '/', strToUTF8( $ice ), $track );
            ...
            if ( !$info = $cache->get( 'stream.' . $chn_key . '.info' ) ) {
                ...
                if ( $db->connect_errno > 0 ) {
                    ...
                    $sam = mysqli_fetch_assoc(
                        $db->query( "SELECT songID, artist, title, date_played, duration FROM {$channel['stats']['db']}.historylist ORDER BY `historylist`.`date_played` DESC LIMIT 0,1"
                        )
                    );
                    ...
                    if ( $db->error ) { // Failed to connect
                        ...
                        // Now, after so much checks and stuff, do track match
                        $info[ 'artist' ] = ( ( empty( $track[ 'artist' ] ) ) ? $settings[ 'artist_default' ] : trim( strToUTF8( $track[ 'artist' ] ) ) );
                        $info[ 'title' ] = ( ( empty( $track[ 'title' ] ) ) ? $settings[ 'title_default' ] : trim( strToUTF8( $track[ 'title' ] ) ) );
            ...
            if ( !$info = $cache->get( 'stream.' . $chn_key . '.info' ) ) {
                ...
                if ( !$centova = get( "{$channel['stats']['url']}/external/rpc.php?m=streaminfo.get&username={$channel['stats']['user']}&rid={$channel['stats']['user']}&charset=utf8" ) ) {
                    ...
                    if ( !empty( $centova[ 'error' ] ) ) {
                        ...
                        // We don't use strToUTF8 here because JSON doesn't support non-utf8 characters
            ...
            if ( !$info = $cache->get( 'stream.' . $chn_key . '.info' ) ) {
                ...
                if ( !$onmy = get( "http://api.radionomy.com/currentsong.cfm?radiouid={$channel['stats']['user-id']}&apikey={$channel['stats']['api-key']}&callmeback=yes&type=xml&cover=yes&previous=no" ) ) {
                    ...
                    if ( $radioxml === false || !is_array( $radioxml ) ) {
                        ...
                        // Now, after so much checks and stuff, do track match
                        $info[ 'artist' ] = strToUTF8( ( empty( $track[ 'artists' ] ) ) ? $settings[ 'artist_default' ] : trim( $track[ 'artists' ] ) );
                        $info[ 'title' ] = strToUTF8( ( empty( $track[ 'title' ] ) ) ? $settings[ 'title_default' ] : trim( $track[ 'title' ] ) );
            ...
            if ( !$info = $cache->get( 'stream.' . $chn_key . '.info' ) ) {
                ...
                if ( !$txt = get( $channel[ 'stats' ][ 'url' ], false, ( ( !empty( $channel[ 'stats' ][ 'user' ] ) && !empty( $channel[ 'stats' ][ 'pass' ] ) ) ? "{$channel['stats']['user']}:{$channel['stats']['pass']}" : '' ) ) ) {
                    ...
                    if ( $txt === false || empty( $txt ) ) {
                        ...
                        if ( ( $track = json_decode( strToUTF8( $txt ), true ) ) === null ) {

                            // Now, after so much checks and stuff, do track match
                            preg_match( '/' . $settings[ 'track_regex' ] . '/', strToUTF8( $txt ), $track );
